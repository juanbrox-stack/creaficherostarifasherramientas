...
def extraer_precios_por_posicion(df, col_ref_idx, col_precio_idx):
    mapping = {}
    if df is range or df is None or df.empty:
        return mapping
...
# Helper para convertir índice a letra Excel (0->A, 1->B...)
def letra_columna(n):
    string = ""
    while n >= 0:
...
# =========================================================================
# BARRA LATERAL - CARGA DE ARCHIVOS MAESTROS Y AUDITORÍA DE ESTRUCTURA
# =========================================================================
...
# Pestañas críticas esperadas por defecto
pestañas_nac_esperadas = ["T_AMZ", "T_MM", "T_MIR", "T_C4"]
pestañas_int_esperadas = ["ES-FR", "ES-IT", "ES-DE", "PT", "NL", "BE", "PL", "SE"]

if archivo_nac:
    try:
        xl_nac = pd.ExcelFile(archivo_nac)
        pestañas_detectadas = xl_nac.sheet_names
        for sheet in pestaña_detectadas:
            pestañas_nac[sheet] = xl_nac.parse(sheet, header=None)
        
        # Alerta de Variación de Estructura: Pestañas nuevas o ausentes
        ...
        if pestañas_faltantes:
            st.sidebar.error(f"⚠️ ¡Alerta Estructura Nacional! Faltan pestañas críticas: {pestañas_faltantes}")
        if pestañas_nuevas:
            st.sidebar.warning(f"💡 Se detectaron pestañas nuevas en el archivo Nacional: {pestañas_nuevas}")
        if not pestañas_faltantes:
            st.sidebar.success("✅ Estructura de pestañas Nacionales validada correctamente.")
...
if archivo_int:
    try:
        ...
        if pestañas_faltantes_int:
            st.sidebar.error(f"⚠️ ¡Alerta Estructura Internacional! Faltan pestañas críticas: {pestañas_faltantes_int}")
        if pestañas_nuevas_int:
            st.sidebar.warning(f"💡 Se detectaron pestañas nuevas en el archivo Internacional: {pestañas_nuevas_int}")
        if not pestañas_faltantes_int:
            st.sidebar.success("✅ Estructura de pestañas Internacionales validada.")
    except Exception as e:
        st.sidebar.error(f"Error al leer Tarifa Internacional: {e}")

# =========================================================================
# MÓDULO INTERACTIVO DE REMAPEO DE COLUMNAS (CONTROLES DINÁMICOS)
# =========================================================================
st.write("---")
with st.expander("🛠️ Panel Avanzado de Mapeo y Remapeo de Columnas (Si cambia la estructura del Excel)"):
    st.write("Si el proveedor ha añadido o movido columnas, selecciona manualmente a qué posición corresponde cada dato:")
    
    c_map1, c_map2 = st.columns(2)
    
    with c_map1:
        st.markdown("**📌 Canales Nacionales**")
        # Conseguir una fila de ejemplo para mostrar cabeceras o letras
        ...
        re_pvp_mm  = st.selectbox("Columna PVP PUB para T_MM (MediaMarkt / PcComp):", opciones_cols_nac, index=16)
    ...
    with c_map2:
        ...
        sample_int = pestañas_int.get("ES-FR") if "ES-FR" in pestañas_int else (next(iter(pestañas_int.values())) if pestañas_int else None)
...
# Estructura horizontal limpia de 18 columnas para la plantilla del Cargador de Precios
columnas_plantilla = [
...
# =========================================================================
# VISTA DE PESTAÑAS DE TRABAJO (BLOQUES PRINCIPALES)
# =========================================================================
...
with tab1:
    ...
    reglas_caracteristicas = {
        ...
        "Prix_France": ("Internacional", ["ES-FR"], idx_ref_int, idx_pvp_int),
        "PVP_SHEIN_FR": ("Internacional", ["ES-FR"], idx_ref_int, idx_pvp_int),
        "Prix_Italia": ("Internacional", ["ES-IT"], idx_ref_int, idx_pvp_int),
        "PVP_SHEIN_IT": ("Internacional", ["ES-IT"], idx_ref_int, idx_pvp_int),
        "prix_Alemania": ("Internacional", ["ES-DE"], idx_ref_int, idx_pvp_int),
        "PVP_SHEIN_DE": ("Internacional", ["ES-DE"], idx_ref_int, idx_pvp_int),
        "PVP_SHEIN_PT": ("Internacional", ["PT"], idx_ref_int, idx_pvp_int),
        "PVP PORTUGAL": ("Internacional", ["PT"], idx_ref_int, idx_pvp_int),
        "PVP_BE": ("Internacional", ["BE"], idx_ref_int, idx_pvp_int),
        "PRIXHOLANDA": ("Internacional", ["NL"], idx_ref_int, idx_pvp_int),
        "PVP_SHEIN_NL": ("Internacional", ["NL"], idx_ref_int, idx_pvp_int),
        "PVP_SHEIN_PL": ("Internacional", ["PL"], idx_ref_int, idx_div_pub),
        "PRIX_POLONIA": ("Internacional", ["PL"], idx_ref_int, idx_div_pub),
        "PVP Suecia": ("Internacional", ["SE"], idx_ref_int, idx_div_pub)
    }
    ...
    if st.button("📦 Generar todas las Características"):
        if not archivo_nac or not archivo_int:
            ...
            if diccionario_archivos:
                ...
                st.error("No se pudo extraer información con datos válidos. Verifique el mapeo de columnas.")
...
with tab2:
    ...
    st.write("Presiona el botón para procesar las tarifas horizontales aplicando la limpieza de filas vacías basada en tu mapeo.")
    ...
    if st.button("🚀 Procesar y Preparar Ficheros del Cargador"):
        ...
        if not archivo_nac or not archivo_int or df_amz is None:
            ...
            map_fr = buscar_y_extraer(["ES-FR"], idx_ref_int, idx_pvp_int)
            map_it = buscar_y_extraer(["ES-IT"], idx_ref_int, idx_pvp_int)
            map_de = buscar_y_extraer(["ES-DE"], idx_ref_int, idx_pvp_int)
            map_nl = buscar_y_extraer(["NL"], idx_ref_int, idx_pvp_int)
            map_be = buscar_y_extraer(["BE"], idx_ref_int, idx_pvp_int)
            map_pl_zlotis = buscar_y_extraer(["PL"], idx_ref_int, idx_div_pub)
            ...
            st.success("¡Los 3 archivos del cargador se han procesado de forma estricta y limpia!")
