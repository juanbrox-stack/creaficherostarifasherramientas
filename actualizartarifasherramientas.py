...
# Añadida la columna técnica 'price_se' para incluir de forma correcta la tarifa de Suecia
columnas_plantilla = [
    'reference', 'price_france', 'price_italy', 'price_germany', 'price_portugal',
    'price_spain', 'price_poland', 'price_holand', 'price_se', 'price_tradeinn_es', 'price_aliexpress_es',
    ...
    if st.button("📦 Generar todas las Características"):
        if not archivo_nac or not archivo_int:
            ...
            if diccionario_archivos:
                ...
                st.error("No se pudo extraer información con datos válidos. Revisa la estructura de los archivos.")
...
with tab2:
    ...
    if st.button("🚀 Procesar y Preparar Ficheros del Cargador"):
        ...
        if not archivo_nac or not archivo_int or df_amz is None:
            ...
            # Purgar filas completamente vacías en el bloque de España
            ...
            # --- PROCESAMIENTO FICHERO 3: RESTO EUROPA (Incluye BE y SE) ---
            ...
            map_be = buscar_y_extraer(["BE"], 2, 12)       # Extraído de la pestaña BE (Col C y M)
            map_pl_zlotis = buscar_y_extraer(["PL"], 2, 13)
            map_se_coronas = buscar_y_extraer(["SE"], 2, 13) # Extraído de la pestaña SE (Col C y N)

            # Asignación a DataFrames
            ...
            # Holanda y Bélgica comparten la columna price_holand (Cruce seguro sin sobrescribir si una falta)
            ...
            df_f3['price_se'] = df_f3['reference'].map(map_se_coronas) # Inyección en la columna destino de Suecia
            
            # Purgar filas completamente vacías de este bloque
